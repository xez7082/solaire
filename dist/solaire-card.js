(function() {
  ...
  class SolaireCard extends LitElement {
    ...
    _renderItem(p) {
      ...
      const isActive = p.startsWith('w')
        ? false
        : (Math.abs(parseFloat(val1)) > (c.flow_th || 2));

      const bCol = c[p+'_bc'] || '#4caf50';
      const isTransBorder = bCol === 'transparent' || bCol === 'none';

      return html`
        <div class="item-box ${isActive && !isTransBorder ? 'animated-border' : ''}"
          style="
            left:${c[p+'_x']}px;
            top:${c[p+'_y']}px;
          ...
          <div class="inner-card"
            style="background:${c[p+'_bg'] || 'rgba(15,15,15,0.85)'};border-radius:${c[p+'_br'] || 12}px;">
    ...
    static get styles() {
      return css`
        ...
        .item-box{position:absolute;padding:var(--border-thickness);display:flex;box-sizing:border-box;pointer-events:auto;}
        .inner-card{display:flex;align-items:center;justify-content:center;padding:10px;width:100%;backdrop-filter:blur(5px);height:100%;box-sizing:border-box;}
    ...
    _renderTabContent(ents){
      ...
      if(t==='gen') return html`
        <div style="display:grid;gap:10px;">
          Fond URL:
          <input type="text" .value=${c.background_image||''}
            @input=${e=>this._up('background_image',e.target.value)}>

          W/H Carte:
          <div style="display:flex;gap:5px;">
            <input type="number" .value=${c.card_width}
              @input=${e=>this._up('card_width',e.target.value)}>

            <input type="number" .value=${c.card_height}
