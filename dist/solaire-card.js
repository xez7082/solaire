(function() {
  ...
  class SolaireCard extends LitElement {
    ...
    render() {
      ...
      return html`
        <ha-card style="width:${w}px; height:${h}px; border:1px solid ${c.border_color||'#00ffff'}; background: url('${c.background_image}') no-repeat; background-size:100% 100%; position:relative; overflow:hidden;">
    ...
    _drawFlow(i) {
      ...
      const s = c['f'+i+'_s'];
      const val = s && this.hass.states[s] ? parseFloat(this.hass.states[s].state) : 500; // 500 par défaut pour test
      if (val === 0 && s) return '';
      ...
      return html`
        <g>
          <path d="${c['f'+i+'_p']}" fill="none" stroke="${color}" stroke-width="3" stroke-dasharray="6,20" stroke-linecap="round">
            <animate attributeName="stroke-dashoffset" from="${val >= 0 ? 100 : 0}" to="${val >= 0 ? 0 : 100}" dur="${speed}s" repeatCount="indefinite" />
          </path>
        </g>`;
    }
    ...
    _renderBat(p) {
      ...
      return html`<div class="sensor-block" style="left:${c[p+'_x']}px; top:${c[p+'_y']}px; transform:rotate(${c[p+'_rot']||0}deg); transform-origin:top left;">
          ...
          <div class="bar" style="width:${c[p+'_w']||80}px; height:${c[p+'_h']||10}px; border:1px solid ${c[p+'_color']||'white'}"><div style="width:${soc}%; background:${soc>20?'#4caf50':'#f44336'}; height:100%"></div></div>
      </div>`;
    }
    ...
    static get styles() { return css`.sensor-block{position:absolute;font-weight:bold;text-shadow:2px 2px 4px black;white-space:nowrap;line-height:1.1;display:flex;flex-direction:column;pointer-events:none}.sensor-name{font-size:0.65em;opacity:0.8;text-transform:uppercase}.bar{background:rgba(0,0,0,0.5);border-radius:2px;overflow:hidden;margin-top:2px}`; }
  }
  customElements.define("solaire-card", SolaireCard);
  window.customCards = window.customCards || [];
  window.customCards.push({ type: "solaire-card", name: "Solaire Master V14", preview: true });
})();
