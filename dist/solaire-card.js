(function() {
  ...
  class SolaireCard extends LitElement {
    ...
    _draw() {
      ...
      for (let i = 1; i <= 20; i++) {
        ...
        if (!pD) continue;
        const stateObj = this.hass.states[s];
        const v = stateObj ? parseFloat(stateObj.state) : 0;
    ...
    render() {
      const c = this.config;
      const keys = [];
      // Génération de tous les slots S1-10, H1-10, B1-5
      for(let i=1; i<=10; i++) { keys.push(`s${i}`, `h${i}`); if(i<=5) keys.push(`b${i}`); }
      
      return html`
        <ha-card style="width:${c.card_width}px; height:${c.card_height}px; background: #000;">
    ...
    _renderItem(p) {
      const c = this.config;
      // Affichage forcé si X et Y sont définis, même sans entité
      if (c[p + '_x'] === undefined || c[p + '_y'] === undefined) return '';
      
      const s1 = this.hass.states[c[p + '_ent']];
      const s2 = this.hass.states[c[p + '_ent2']];
      
      const val1 = s1 ? parseFloat(s1.state) : 0;
      const val2 = s2 ? parseFloat(s2.state) : 0;
      ...
      return html`
        <div class="item-box ${isActive && !isTransBorder ? 'animated-border' : ''}" style="
          ...
          background: ${isTransBorder ? 'none' : ''};
          box-shadow: ${isTransBorder ? 'none' : (c[p+'_sh'] || '0 8px 16px rgba(0,0,0,0.5)')};
        ">
          ...
          ">
            ...
            <div class="content">
              ...
              ${c[p+'_ent2'] ? html`<div class="value2" style="color:${c[p+'_v2c'] || '#4caf50'}; font-size:${c[p+'_fs_v2'] || 13}px;">${val2.toFixed(1)}${c[p+'_u2'] || ''}</div>` : ''}
            </div>
          </div>
        </div>`;
    }
    ...
    static get styles() { return css`
      ha-card { position: relative; overflow: hidden; border: none; }
  ...
  class SolaireCardEditor extends LitElement {
    ...
    render() {
      ...
      return html`
        <div style="background:#1a1a1a; color:#eee; padding:15px; font-family:sans-serif;">
          ...
          <div style="max-height: 600px; overflow-y: auto;">${this._renderTabContent(ents)}</div>
        </div>`;
    }
    ...
    _renderTabContent(ents) {
      ...
      if (t === 'gen') return html`<div style="display:grid; gap:10px;">
        ...
        Vitesse Flux: <input type="number" .value="${c.flow_speed}" @input="${e=>this._up('flow_speed',e.target.value)}" step="0.1">
      </div>`;
      ...
      return pfx.map(p => html`<details style="background:#222; margin-bottom:5px; padding:8px; border-radius:4px;"><summary>Objet ${p.toUpperCase()}</summary>
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:10px;">
          ...
          Position X: <input type="number" .value="${c[p+'_x']}" @input="${e=>this._up(p+'_x',e.target.value)}">
          Position Y: <input type="number" .value="${c[p+'_y']}" @input="${e=>this._up(p+'_y',e.target.value)}">
          Largeur Boite: <input type="number" .value="${c[p+'_w_box']||120}" @input="${e=>this._up(p+'_w_box',e.target.value)}">
          Hauteur Boite: <input type="number" placeholder="auto si vide" .value="${c[p+'_h_box']||''}" @input="${e=>this._up(p+'_h_box',e.target.value)}">
          Entité 1 (W): <input list="e" .value="${c[p+'_ent']||''}" @input="${e=>this._up(p+'_ent',e.target.value)}">
          Entité 2 (V/%): <input list="e" .value="${c[p+'_ent2']||''}" @input="${e=>this._up(p+'_ent2',e.target.value)}">
          Unités (1 / 2): <div style="display:flex;gap:2px;"><input type="text" .value="${c[p+'_u']||'W'}" @input="${e=>this._up(p+'_u',e.target.value)}"><input type="text" .value="${c[p+'_u2']||''}" @input="${e=>this._up(p+'_u2',e.target.value)}"></div>
          Fond (RGBA): <input type="text" placeholder="transparent" .value="${c[p+'_bg']||'rgba(15,15,15,0.85)'}" @input="${e=>this._up(p+'_bg',e.target.value)}">
          Néon (Hex): <input type="text" placeholder="transparent" .value="${c[p+'_bc']||'#4caf50'}" @input="${e=>this._up(p+'_bc',e.target.value)}">
  ...
  window.customCards.push({ type: "solaire-card", name: "Solaire V170 Full-Control" });
})();
