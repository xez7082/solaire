(function() {
  ...
  class SolaireCard extends LitElement {
    ...
    static get styles() { return css`
      .item{position:absolute; display:flex; flex-direction:column; align-items:center; text-shadow: 1px 1px 3px #000; white-space:nowrap; font-family: sans-serif; transition: 0.3s;}
      .box-style{background: rgba(0,0,0,0.5); padding: 8px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); backdrop-filter: blur(2px);}
  ...
  class SolaireCardEditor extends LitElement {
    ...
    render() {
      ...
      return html`
        <div style="background:#121212; color:#fff; padding:15px; font-family: sans-serif;">
          <div style="display:flex; flex-wrap:wrap; gap:5px; margin-bottom:15px;">
            ${tabs.map(t => html`<button @click="${() => this._tab=t.id}" style="flex:1 1 30%; padding:10px; background:${this._tab===t.id?t.c:'#333'}; color:${this._tab===t.id?'#000':'#fff'}; border:none; cursor:pointer; font-size:11px; font-weight:bold; border-radius:5px; box-shadow: ${this._tab===t.id?'0 0 10px '+t.c:'none'};">${t.label}</button>`)}
          </div>
    ...
    _renderTab(ents) {
      ...
      return (g||[]).map(p => html`<details style="background:#252525; margin-top:5px; border:1px solid #444;"><summary style="padding:8px; cursor:pointer; color:${col}">${p.toUpperCase()} ${c[p+'_ent']?'✅':''}</summary><div style="padding:10px; display:grid; gap:8px;">Nom <input type="text" .value="${c[p+'_name']||''}" @input="${e=>this._up(p+'_name',e.target.value)}"> Watts <input list="el" .value="${c[p+'_ent']||''}" @input="${e=>this._up(p+'_ent',e.target.value)}" style="width:100%; background:#333; color:#fff; padding:5px;">kWh Jour <input list="el" .value="${c[p+'_ent_day']||''}" @input="${e=>this._up(p+'_ent_day',e.target.value)}" style="width:100%; background:#333; color:#fff; padding:5px;">X <input type="number" .value="${c[p+'_x']}" @input="${e=>this._up(p+'_x',e.target.value)}"> Y <input type="number" .value="${c[p+'_y']}" @input="${e=>this._up(p+'_y',e.target.value)}"> Rot <input type="number" .value="${c[p+'_rot']}" @input="${e=>this._up(p+'_rot',e.target.value)}"><br>Cadre <input type="checkbox" .checked="${c[p+'_box']}" @change="${e=>this._up(p+'_box',e.target.checked)}"> Couleur Titre <input type="color" .value="${c[p+'_tc']||'#aaa'}" @input="${e=>this._up(p+'_tc',e.target.value)}"> Valeur <input type="color" .value="${c[p+'_vc']||'#fff'}" @input="${e=>this._up(p+'_vc',e.target.value)}">${p.startsWith('b')?html`Jauge W <input type="number" .value="${c[p+'_w']}" @input="${e=>this._up(p+'_w',e.target.value)}"> H <input type="number" .value="${c[p+'_h']}" @input="${e=>this._up(p+'_h',e.target.value)}">`:''}</div></details>`);
    }
  }
  customElements.define("solaire-card-editor", SolaireCardEditor);
  customElements.define("solaire-card", SolaireCard);
  window.customCards = window.customCards || [];
  window.customCards.push({ type: "solaire-card", name: "Solaire Master V30 BOX", preview: true });
})();
